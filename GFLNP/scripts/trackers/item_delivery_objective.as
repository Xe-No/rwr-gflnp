...
class ItemDeliveryObjective : Objective {
	...
	protected void processCollapsedDrop() {
		...
		int acceptedAmount = min(m_collapseDropAmount, 8);
		int unexpectedAmount = m_collapseDropAmount - acceptedAmount;
		if(unexpectedAmount > 0){
			addBoxesToBackpack(m_metagame, id, targetItem.m_type, targetItem.m_key, unexpectedAmount);
		}
	...
	void addBoxesToBackpack(Metagame@ metagame, int characterId, string itemClass, string key, int num) {
		...
		XmlElement k("item");
		k.setStringAttribute("class", itemClass);
		k.setStringAttribute("key", key);
		for(int i = 0;i < num; i++){
