...
//when an Architect gun run is requested, the caller's id, the call's position and id, the strike's direction and the marker vehicle's id are stored
class ArchitectRequest {
	int m_characterId;
	Vector3 m_targetPos;
	Vector3 m_ArchitectPos;
	int m_direction;
	int m_callId;
	int m_factionId;

	ArchitectRequest(int characterId, Vector3 targetPos, Vector3 ArchitectPos, int direction, int callId, int factionId){
		m_characterId = characterId;
		m_targetPos = targetPos;
		m_ArchitectPos = ArchitectPos;
...
class ArchitectGunRun : Tracker {
	...
	protected Vector3 m_Architect_pos;



	protected bool m_running = false;
	protected array<ArchitectRequest@> ArchitectQueue;
	protected bool m_shadow = false;
	protected float m_pi = acos(-1.0f);


	ArchitectGunRun(Metagame@ metagame) {
		@m_metagame = @metagame;
	}
	...
	protected void handleCallEvent(const XmlElement@ event) {
		...
		if (call_key == "architect_drop.call" ) {
			...
			if (phase == "queue") {
				...
				int direction = 9;
				Vector3 ArchitectPos = targetPos.add(Vector3(0.0,30.0,0.0));

				ArchitectRequest@ thisCall = ArchitectRequest(characterId, targetPos, ArchitectPos, direction, callId, factionId);

				//placing ground marker and flag
				//addMarker(thisCall);
				//the queue is necessary to handle multiple simultaneous Architect requests
				ArchitectQueue.insertLast(thisCall);
			}
	...
	void update(float time) {
		...
		if (m_timer < 0.0) {
			if (m_timer_attack < 3.0) {
				...
				if (m_timer_25 <= 0.0){
					antiPerson(ArchitectQueue[0], 1, "architect_gl.projectile", "ac130_105mm.wav");
					m_timer_25 = 0.5;
				}

				// if (m_timer_40 <= 0.0){
				// 	antiPerson(ArchitectQueue[0], 1, "ac130_40mm.projectile", "ac130_40mm.wav");
				// 	m_timer_40 = 1.5;
				// }

				// if (m_timer_105 <= 0.0){
				// 	antiPerson(ArchitectQueue[0], 1, "ac130_105mm.projectile", "ac130_105mm.wav");
				// 	m_timer_105 = 6.0;
				// }
				//removeMarker(ArchitectQueue[0]);
	...
	void addMarker(ArchitectRequest@ ArchitectRequest) {
		//placing the flag
		Vector3 targetPos = ArchitectRequest.m_targetPos;
		int characterId = ArchitectRequest.m_characterId;
		Vector3 direction = gunRunVector(ArchitectRequest.m_direction);		
		int flagId = ArchitectRequest.m_callId + 8000;

		XmlElement command("command");
		command.setStringAttribute("class", "set_marker");
		command.setIntAttribute("id", flagId);
		command.setIntAttribute("faction_id", ArchitectRequest.m_factionId);
		...
		// command.setStringAttribute("type_key", "call_marker_a10_" + ArchitectRequest.m_direction);
	...
	void removeMarker(ArchitectRequest@ ArchitectRequest) {
		int flagId = ArchitectRequest.m_callId + 8000;
	...
	void antiPerson(ArchitectRequest@ ArchitectRequest, int number, string instanceKey, string soundFile) {
		// Now we find the line perpendicular to caller-target

		//extracting data
		int characterId = ArchitectRequest.m_characterId;
		int factionId = ArchitectRequest.m_factionId;
		Vector3 targetPos = ArchitectRequest.m_targetPos;
		Vector3 direction = gunRunVector(ArchitectRequest.m_direction);
		Vector3 ArchitectPos = ArchitectRequest.m_ArchitectPos;
		...
		for (int f = 0; f < 6; ++f){
			...
			if (f!= factionId){
				...
				if (soldier_id >= 0){
					...
					P2PAttack(ArchitectPos, soldier_pos, number, instanceKey, soundFile, spread, factionId, characterId);
				}
			}
		}
	}
