...
class LocalBanManager : Tracker {
	...
	protected void handleChatEvent(const XmlElement@ event) {
		...
		else if (checkCommand(message, "tempban1")) {
			handleTempBan(message, senderId, false, 86400.0);
		} 
		else if (checkCommand(message, "tempban3")) {
			handleTempBan(message, senderId, false, 86400.0 * 3.0);
		} 
		else if (checkCommand(message, "tempban7")) {
			handleTempBan(message, senderId, false, 86400.0 * 7.0);
		} 
		else if (checkCommand(message, "tempban")) {
			handleTempBan(message, senderId, false, 1800.0);
		} 
	...
	// --------------------------------------------
	void handleTempBan(string message, int senderId, bool id = false, float banTime = 1800.0) {
